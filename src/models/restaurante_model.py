# src/models/restaurante_model.py
...
class Restaurante(db.Model):
    ...
    nome = db.Column(db.String(100), unique=True, nullable=False) # Nome do restaurante, deve ser único
    cnpj = db.Column(db.String(18), unique=True, nullable=False) # CNPJ com máscara ou sem, mas único
    telefone = db.Column(db.String(20), nullable=True) # Telefone de contato do restaurante
    email = db.Column(db.String(120), unique=True, nullable=True) # Email do restaurante, também único
    
    descricao = db.Column(db.Text, nullable=True) # Breve descrição do restaurante
    categoria = db.Column(db.String(50), nullable=True) # Ex: 'Pizzaria', 'Japonesa', 'Brasileira'
    ...
    # Chave estrangeira para Endereco (mantendo sua estrutura)
    ...
    # Construtor explícito para facilitar a criação de instâncias
